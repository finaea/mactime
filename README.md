...
- **Exports and imports history** — A backup file supports moving an existing history to another Mac or keeping a copy outside MacTime.
...
The key remains on the Mac where the history was created. Copying the MacTime data folder to another Mac does not make the history readable there. If the key is unavailable, MacTime stops recording rather than creating a separate unreadable history. Existing history can be opened by restoring the original login keychain or importing an earlier backup; otherwise, Settings → Delete data → Everything starts a new history.

### Back up or move history

Migration Assistant and a full Time Machine restore carry both the login keychain and MacTime history, so no additional MacTime step is needed.

For a clean install, keychain reset, or move to another Mac, use **Settings → Backup**:

- **Export…** saves screenshots and activity history in one backup file.
- **Import…** replaces the history on this Mac with a selected backup, then restarts MacTime.

MacTime confirms these actions with Touch ID or a password, and Settings shows when the last export was created.

**Exports are not encrypted.** They contain screenshots and activity history in a readable form, so they should be stored with the same care as the original history.
